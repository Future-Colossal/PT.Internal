...
/*
This example will receive multiple universes via Artnet and control a strip of ws2811 leds1 via 
...
// Neopixel settings
const int numLeds1 = 220;                   // change for your setup
const int numLeds2 = 220;                   // change for your setup
const int numberofChannels1 = numLeds1 * 4;  // Total number of channels you want to receive (1 led = 3 channels)
const int numberofChannels2 = numLeds2 * 4;  // Total number of channels you want to receive (1 led = 3 channels)
const byte dataPin1 = 3;
const byte dataPin2 = 4;
Adafruit_NeoPixel leds1 = Adafruit_NeoPixel(numLeds1, dataPin1, NEO_GRBW + NEO_KHZ800);
Adafruit_NeoPixel leds2 = Adafruit_NeoPixel(numLeds2, dataPin2, NEO_GRBW + NEO_KHZ800);

// Artnet settings
Artnet artnet;
const int startUniverse11 = 0;  // CHANGE FOR YOUR SETUP most software this is 1, some software send out artnet first universe as 0.
...
const int maxUniverses = maxUniverses1 + maxUniverses2;
...
byte mac[] = { 0x04, 0xE9, 0xE5, 0x14, 0x44, 0x27 };
...
void onDmxFrame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t* data) {
  ...
  if ((universe - startUniverse1) < maxUniverses)
    universesReceived[universe - startUniverse1] = 1;
  ...
  for (int i = 0; i < length / 4; i++) {
    int led = i + (universe - startUniverse1) * (previousDataLength / 4);
    leds1.setPixelColor(led, data[i * 4], data[i * 4 + 1], data[i * 4 + 2]);
