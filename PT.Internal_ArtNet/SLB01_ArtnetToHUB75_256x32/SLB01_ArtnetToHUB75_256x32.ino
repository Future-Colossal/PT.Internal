...
// Make sure to set the CPU Speed to "816 MHz (Overclock)" in the "Tools" menu. 
...
// Neopixel settings
const int numLeds = kMatrixWidth * kMatrixHeight;                 // change for your setup
...
void onDmxFrame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t* data) {
  ...
  for (int i = 0; i < length / 3; i++) {
    ...
    row = led / kMatrixWidth;
    backgroundLayer.drawPixel(led % kMatrixWidth, row, pixel_color);
    // if (led <= numLeds) {
    //   backgroundLayer.drawPixel(led % kMatrixWidth, row, pixel_color);
    // }
    // if (led>63 && led <= 128) {
